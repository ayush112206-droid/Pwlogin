...
async function startServer() {
  ...
  // Proxy for Extraction API to bypass CORS
  app.all("/api/v1/proxy/extract", async (req, res) => {
    try {
      // Support both GET (query params) and POST (body)
      const mpd_url = req.query.mpd_url || req.body?.mpd_url;
      const bearer_token = req.query.bearer_token || req.body?.bearer_token;
      
      if (!mpd_url || !bearer_token) {
        return res.status(400).json({ error: "mpd_url and bearer_token are required. Example: /api/v1/proxy/extract?mpd_url=...&bearer_token=..." });
      }

      // Generate the required headers for PW Cloudfront
      const pwHeaders = getPWHeaders(bearer_token as string);

      // Pass the token exactly as received (usually without Bearer prefix)
      const formattedToken = (bearer_token as string).replace("Bearer ", "");

      // Try the /extract-query GET endpoint first as it might be more reliable
      let response;
      try {
        response = await axios.get("https://keykid-ca081f5cf60d.herokuapp.com/extract-query", {
          params: {
            url: mpd_url,
        ...
        console.log("GET /extract-query failed, falling back to POST /extract", err.message);
        response = await axios.post("https://keykid-ca081f5cf60d.herokuapp.com/extract", {
          headers: pwHeaders
        }, {
          params: {
            mpd_url,
            bearer_token: formattedToken
          }
        });
      }
      ...
      res.status(error.response?.status || 500).json(error.response?.data || { error: "Failed to extract" });
    }
  });
  ...
  app.post("/api/v1/pw/get-otp", async (req, res) => {
    try {
      ...
      res.status(error.response?.status || 500).json({
        success: false,
        message: errorData?.message || errorData?.error || "A server error has occurred",
        raw: errorData
      });
    }
  });
  ...
  app.get("/api/v1/pw/batch-details/:batchId", async (req, res) => {
    try {
      ...
      console.error("Batch Details Error:", error.response?.data || error.message);
  ...
  app.get("/api/v1/pw/batch-subjects/:batchId", async (req, res) => {
    try {
      ...
      try {
        // Try V1 first
        const response = await axios.get(`https://api.penpencil.co/v1/batches/${batchId}/subjects`, {
          params: { organisationId: PW_ORG_ID },
          headers: getPWHeaders(token),
          timeout: 10000
        });
        ...
        console.log("V1 subjects failed, trying V2...");
        try {
          const response = await axios.get(`https://api.penpencil.co/v2/batches/${batchId}/subjects`, {
            params: { organisationId: PW_ORG_ID },
            headers: getPWHeaders(token),
            timeout: 10000
          });
          ...
          console.log("V2 subjects failed, trying V3...");
      ...
      console.error("Batch Subjects Error:", error.response?.data || error.message);
  ...
  app.get("/api/v1/pw/subject-contents/:batchId/:subjectId", async (req, res) => {
    try {
      ...
      try {
        const response = await axios.get(`https://api.penpencil.co/v2/batches/${batchId}/subject/${subjectId}/contents`, {
        ...
        console.log("V2 contents failed, trying V1...");
        try {
          const response = await axios.get(`https://api.penpencil.co/v1/batches/${batchId}/subject/${subjectId}/contents`, {
            params: { organisationId: PW_ORG_ID, page, contentType, tag: "" },
            headers: getPWHeaders(token),
            timeout: 10000
          });
          ...
          console.log("V1 contents failed, trying V3...");
      ...
      console.error("Subject Contents Error:", error.response?.data || error.message);
